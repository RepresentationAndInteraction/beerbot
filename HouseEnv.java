...
public class HouseEnv extends Environment {
	...
	public static final Literal af = Literal.parseLiteral("at(beerbot,fridge)");
	public static final Literal ao = Literal.parseLiteral("at(beerbot,owner)");
	...
	void updatePercepts() {
		...
		clearPercepts("beerbot");
		clearPercepts("owner");

		// get the beerbot location
		Location lRobot = model.getAgPos(0);

		// add agent location to its percepts
		if (lRobot.equals(model.lFridge)) {
			addPercept("beerbot", af);
		}
		if (lRobot.equals(model.lOwner)) {
			addPercept("beerbot", ao);
		}

		// add beer "status" the percepts
		if (model.fridgeOpen) {
			addPercept("beerbot", Literal.parseLiteral("stock(beer,"+model.availableBeers+")"));
		}
		if (model.sipCount > 0) {
			addPercept("beerbot", hob);
